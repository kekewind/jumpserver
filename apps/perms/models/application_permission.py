...
class ApplicationPermission(BasePermission):
    ...
    @classmethod
    def get_include_actions_choices(cls, tp=None, category=None):
        actions = {Action.ALL, Action.CONNECT}
        if not any([tp, category]):
            choices = [Action.NAME_MAP[action] for action in actions]
            return choices

        if tp:
            category = AppType.get_category(tp)

        if category == AppCategory.db:
            actions.update([Action.DOWNLOAD])
        elif category == AppCategory.cloud:
            actions.update([Action.ALL, Action.CONNECT])
    ...
    @classmethod
    def get_exclude_actions_choices(cls, tp=None, category=None):
        include_choices = cls.get_include_actions_choices(tp, category)
